...
export function loadConfig(contentDir) {
    ...
    // Normalize basePath: ensure it starts with "/" (if non-empty) and has no trailing slash.
    // This lets all downstream code safely concatenate: `${basePath}/rss.xml`, `${basePath}${path}`
    if (siteConfig.basePath) {
        const stripped = siteConfig.basePath.replace(/^\/+/, '').replace(/\/+$/, '');
        siteConfig.basePath = stripped ? '/' + stripped : '';
    }
    ...
    const postsDir = path.resolve(contentDir, siteConfig.dirs.posts);
    const pagesDir = path.resolve(contentDir, siteConfig.dirs.pages);
    const imagesDir = path.resolve(contentDir, siteConfig.dirs.images);
    const extrasDir = path.resolve(contentDir, siteConfig.dirs.extras);
    const templateDir = path.resolve(contentDir, siteConfig.dirs.templates);
    const cssFile = path.resolve(contentDir, siteConfig.dirs.css);
    const publicDir = path.resolve(contentDir, siteConfig.dirs.output);
...
// Resolve a local href to an absolute path with basePath.
// Handles: /images/photo.jpg, ../images/photo.jpg, ./images/photo.jpg
// Passes through external URLs (http/https) unchanged.
function resolveLocalHref(href, basePath) {
...
function createMarkedRenderer(siteConfig) {
    ...
    renderer.image = ({ href, title, text }) => {
        const fixedHref = resolveLocalHref(href, siteConfig.basePath);
        ...
        return `<img src="${escapeHtml(fixedHref)}" alt="${escapeHtml(text)}"${titleAttr ? ` title="${escapeHtml(titleAttr)}"` : ''}${width && height ? ` width="${width}" height="${height}"` : ''}>`;
    };

    renderer.link = function({ href, title, tokens }) {
        const isExternal = href.startsWith('http') && !href.startsWith(siteConfig.siteUrl);
        const fullHref = isExternal ? href : resolveLocalHref(href, siteConfig.basePath);
        ...
        return `<a href="${escapeHtml(fullHref)}"${titleAttr}${targetAttr}>${inner}${suffix}</a>`;
    };
...
function resolveOgImage(frontMatter, htmlContent, siteConfig, defaultOgImage) {
    if (frontMatter.image) {
        ...
        if (imgMatch && imgMatch[1]) {
            // The src is already site-absolute (renderer already applied basePath)
...
export function buildIndex(allPosts, ctx) {
    console.info('⌛️ Processing index and 404...');
...
export function copyAssets(ctx) {
    if (fs.existsSync(ctx.imagesDir)) {
    ...
    if (fs.existsSync(ctx.extrasDir)) {
